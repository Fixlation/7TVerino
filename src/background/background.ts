...
if (!chrome.scripting) {
	...
	void chrome.scripting
		.getRegisteredContentScripts({ ids: [contentScript.id] })
		.then((registered) => {
			if (registered.some((script) => script.id === contentScript.id)) return;

			return chrome.scripting.registerContentScripts([contentScript]);
		})
		.catch((err: unknown) => {
			const message = String(err ?? "");
			if (message.includes("Duplicate script ID")) return;

			log.error("failed to register content scripts", message);
		});
}
