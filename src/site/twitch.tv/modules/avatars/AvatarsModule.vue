...
watch(avatarClass, (ac, oldAc) => {
	...
	defineFunctionHook(ac, "render", function (old, ...args) {
		if (shouldRenderAvatars.value && performance.animatedAvatarsEnabled.value) {
...
watchDebounced(
	avatars,
	() => {
		if (shouldRenderAvatars.value && performance.animatedAvatarsEnabled.value) doRerender();
	},
	{
		debounce: 350,
	},
...
watch([shouldRenderAvatars, performance.animatedAvatarsEnabled], doRerender);
